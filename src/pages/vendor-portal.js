...
function render(){
  ...
  $('#grnBody').innerHTML=GRNS.length?GRNS.map(g=>{const its=itemsFor(GRNITEMS,'grn_id',g.id),acc=its.reduce((a,i)=>a+Number(i.accepted_qty||0),0),rej=its.reduce((a,i)=>a+Number(i.rejected_qty||0),0);return '<tr><td><div class="portal-doc">'+esc(g.grn_no)+'</div><div class="portal-meta">'+esc(g.delivery_challan_no||'No DC')+'</div></td><td>'+esc(byId(POS,g.po_id)?.po_no||'—')+'</td><td>'+esc(byId(PROJECTS,g.project_id)?.name||'—')+'</td><td>'+fmtDate(g.receipt_date)+'</td><td><b>'+acc+'</b> accepted<div class="portal-meta">'+rej+' rejected</div></td><td>'+status(g.status)+'</td></tr>';}).join(''):'<tr><td colspan="6"><div class="portal-empty">No GRNs posted.</div></td></tr>';
